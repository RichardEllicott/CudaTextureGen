...
__global__ void calculate_outflow3(
    ...
    float *_slope_vector2_map = arrays->_slope_vector2_map;
    // ================================================================
    ...
    // float2 slope_vector = {arrays->_slope_vector2_map[idx2], _slope_vector2_map[idx2 + 1]};
    float2 slope_vector = cmath::array::load_float2(_slope_vector2_map, idx2);
    ...
    for (int n = 0; n < 8; ++n) {
        float2 dot_offset = cmath::GRID_OFFSETS_8_DOTS[n];    // dot offset gives a vector to get a scaled product from
        float product = cmath::dot(dot_offset, slope_vector); // dot product gets how strongly we push into this direction
