...
template <typename T>
inline void to_device_array(const nb::ndarray<T, nb::c_contig> &source, core::cuda::DeviceArray<T> &destination) {
    if (source.ndim() != 1) {
...
template <typename T>
inline void to_device_array(const nb::ndarray<T, nb::c_contig> &source, core::cuda::DeviceArray2D<T> &destination) {
    if (source.ndim() != 2) {
...
//     return to_array(std::forward<Args>(args)...);
// }

// template <typename... Args>
// inline auto to_device_array(Args &&...args) {
//     return ::nanobind::helper::to_device_array(std::forward<Args>(args)...);
// }
