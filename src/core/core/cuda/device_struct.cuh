...
template <typename T>
class DeviceSyncedStruct {
    DeviceStruct<T> dev_struct;
    T host_struct{};

  public:
    DeviceSyncedStruct() { dev_struct.upload(host_struct); }
    explicit DeviceSyncedStruct(const T &value) : host_struct(value) { dev_struct.upload(host_struct); }
