...
// using a ref of a device array should allow's python to use Devicerray as a ref type
// this can allow sharing the same GPU memory for different Python tasks
// The same behaviour could be achieved with a shared_ptr but the Ref object handles some more stuff
