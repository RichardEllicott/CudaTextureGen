#include "shader_maps.bind.h"

void bind_shader_maps(nb::module_ &m) {

    auto _class = nb::class_<shader_maps::ShaderMaps>(m, "ShaderMaps")

                      .def(nb::init<>()) // init

                      // bind generate_normal_map
                      .def("generate_normal_map", [](shader_maps::ShaderMaps &self, nb::ndarray<float> arr, float amount, bool wrap) {
                          if (arr.ndim() != 2)
                      ...
                      .def("generate_ao_map", [](shader_maps::ShaderMaps &self, nb::ndarray<float> arr, float radius = 1.0f, bool wrap = true) {
                          if (arr.ndim() != 2)
                              throw std::runtime_error("Expected a 2D float32 array");
